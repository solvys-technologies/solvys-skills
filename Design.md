...
2. For a new Solvys project or greenfield frontend, run `/solvys-discovery` and
   `refero-design` before touching frontend files, writing CSS, generating UI,
   or treating a visual direction as implementation-ready. The plan must cite
   the Refero reference lock and decision ledger. If missing, install it with
...
## Wonder And Source-Transfer Contract

- New frontend feature and app prototypes begin in Wonder whenever the surface
  is representable there.
- Wonder is provisional co-design truth. TP alone controls whether accepted
  Wonder work transfers into product source.
- The agent owns only its recorded Wonder page or artboard and disregards
  concurrent human changes outside that recorded scope.
- Prototypes use installed or provenance-recorded library components instead of
  scratch-building generic UI. BeUI is primary, Vercel UI is secondary, Bklit
  is the primary eligible visualization source, and EvilCharts is secondary.
- Product state, domain behavior, security, persistence, and data cadence remain
  product-owned and protected from presentation-library ownership.
- After TP authorizes source transfer, port 7777 or an explicitly named sandbox
  is the source-integrated proof rung. Wonder cannot satisfy repository,
  deployed, live, release, or installed proof.
- Policy-only work records all four library-foundation decisions as not
...
editing. Load `/solvys-cao` plus the smallest applicable Solvys skills: usually `/solvys-feels`,
`/solvys-transitions`, `/solvys-brief` or `/solvys-orchestrate`, and
`/solvys-test`.
...
- Greenfield work has `/solvys-discovery` plus `refero-design` reference lock.
