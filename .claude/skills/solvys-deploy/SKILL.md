...
- **DMG lands on Desktop rule** (every DMG publish): after electron-builder emits
  the DMG, remove every `Fintheon-*.dmg` already on `~/Desktop/` and copy the
  new one there. TP installs from Desktop; old DMGs confuse it.
...
- Run repository builds and release checks in the Cloud worktree by default.
  At less than 10 GB or 5 percent internal free, do not start a local build,
  preview, server, or dependency install. A local backend restart requires an
  explicit Mac-runtime or installed-app proof need; do not restart it solely
  for a routine deploy.
- Never start a vite dev server -- verify via `tsc --noEmit` + `vite build` only
- For frontend releases, confirm the project-owned ChatGPT Site has the current
  source, the library installation receipt is present, and Site checks plus any
  required automatic `human-review` feedback are complete before deployment.
  The Site must be a 1:1 runnable representation of the accepted source or
  product specification, with real controls, states, responsive behavior,
  accessibility checks, fixture provenance, and an interaction receipt. Do not
  deploy placeholder or fake product behavior.
  Wonder owns new frontend proposals and their diffs before Site deployment.
  Existing Builder and Plasmic artifacts remain legacy inputs unless TP
  explicitly authorized a source transfer. Do not spawn a duplicate local
  dev server.
...
- Frontend-only and combined work must identify the Cloud branch or approved
  local exception, project ChatGPT Site URL, library installation receipt,
  automatic human-review state when applicable, and Site proof.
...
Before checking the deployment URL, verify the current source through the
project ChatGPT Site in the Codex in-app browser. This proof must exercise the
implemented route and its real controls. Confirm that the Site matches the
accepted source/spec at the requested scope, including copy, geometry, states,
responsive behavior, accessibility, and interaction feedback. A local response
or a legacy-editor preview cannot pass this gate.
...
Run this verification inline. If no sprint brief exists, skip to Phase 4. Use
the Codex in-app browser for browser proof and Plannotator for review receipts;
do not dispatch a separate proof skill.
...
- Keep routine builds and release checks in the Cloud worktree. Restart a local
  backend only for an explicit Mac-runtime or installed-app proof need.
- Never start a vite dev server. Verify via build only.
- Reuse the project ChatGPT Site for source-integrated frontend verification.
  Run `human-review` automatically on a Site-derived local HTML artifact when
  visual or content review is needed.
- Do not deploy a Site placeholder, screenshot, fake dashboard, invented data,
  dead control, skeleton-only preview, or close-enough substitute as the
  product surface.
