...
3. If the issue is for a new Solvys project or greenfield frontend, include a blocking frontend gate in the description: `/solvys-discovery` and `refero-design` must produce a reference lock before anyone touches frontend files, writes CSS, generates UI, or treats visual direction as implementation-ready.
4. Create the issue with: title, description, state (Backlog/Todo), priority, labels
5. Return the issue URL for the user
...
Fintheon's Plane workspace should mirror the canonical feature structure from the repo instruction files, including legacy `CLAUDE.md` when present:
...
- **refero-design**: Required through `/solvys-discovery` before any greenfield/new-project frontend implementation issue can move past planning
