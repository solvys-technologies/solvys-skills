...
- Choose the implementation stack from the current repository and approved
  backend/UI source ledgers.
- Default product UI work to existing repository primitives and approved
  library blocks.
- Record the source and component choice in the brief before implementation.
- If the requested work needs a new framework, auth provider, icon runtime, ORM,
  UI kit, animation runtime, or deployment model, call that out as out-of-canon
  and require explicit TP approval.
