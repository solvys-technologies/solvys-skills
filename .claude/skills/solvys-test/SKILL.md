...
For new frontend changes, first confirm the accepted Wonder decision was
explicitly transferred into product source. Disregard unrelated concurrent
human changes from Wonder. Run the integrated local proof through the existing
port 7777 lane before treating deployed browser proof as representative.
...
For Solvys UI changes, also verify against `Design.md`: no gradients as fills, no emojis, no Kanban side-stripe borders, no AI sparkles, no generic shadows, no decorative button borders/backplates, no pointed square borders or triangular corner flags, no homemade Liquid Glass without a source-backed shipped example, no duplicate or developer-facing UI copy, no raw source values without proper user-facing capitalization, no invented icons outside the app facade/Lucide/Nucleo/verified brand SVG path, responsive behavior at mobile and desktop widths, visible loading/empty/error states, tabular numbers for data, enter/exit transitions for new popups/rails/drawers/modals/sheets/panels, and Solvys-native glass, fading rulers, spacing, type, or flat-row surfaces.
...
When the brief names a Linear issue in Awaiting Review, treat this PASS as a
review decision only after the report proves the original problem is resolved,
all requested/planned controls work through their applicable states, and the
design gate passes or is explicitly not applicable. Comment that evidence and
move the issue to the team's completed state. Leave PARTIAL/FAIL issues in
review and route each failed gate back into sprint work.
