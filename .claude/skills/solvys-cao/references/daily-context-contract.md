...
- Internal capacity, Cloud execution reservations, Ext recovery state, the
  tranche/workspace registry, ChatGPT Site review state, and ten-day
  conversation sweep candidates.
...
The default device scope covers the internal managed-worktree root, Cloud return
branches, SSFitness, the HRight operating workspace, the Solvys Ops control
room, and the recorded ShipTracker workspace. Ext paths remain visible as
protected recovery references, never as default active repositories. Missing or
non-Git workspaces must remain visible in the output instead of disappearing
from the daily picture.
...
11. **Storage and execution lanes** - internal capacity, Cloud routing, Ext
    recovery state, ChatGPT Site review state, workspace closure state,
    protected conversations, and transcript candidates that crossed the 30-day
    inactivity/non-reference gate.
...
- A localhost route, port 7777, Plasmic preview, or Site screenshot is never
  reported as a higher proof rung than it reached.
- A frontend sandbox has no unique ChatGPT Site or a required human review has
