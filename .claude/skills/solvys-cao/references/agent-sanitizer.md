...
9. Classify custody before backup: Ext remains recovery-only, personal media
   stays outside Solvys authority unless TP explicitly selects it, and Solvys
   code uses Git plus bounded dirty-overlay preservation. For an explicitly
   selected Cloud backup, verify its encrypted manifest and restore/readback
   proof before any destructive action against that source. Never expand the selected set.
