...
REQUIRED = [
    ...
    "references/chatgpt-site-review.md",
    "references/plasmic-frontend-editor.md",
...
REQUIRED_PHRASES = {
    "SKILL.md": [
        ...
        "restored writable recovery asset",
    ...
    "references/storage-and-execution-lanes.md": [
        ...
        "An authorized bounded backup",
    ],
    "references/plasmic-frontend-editor.md": [
        "Legacy Visual Editor Contract",
        "ChatGPT Site",
    ...
    "references/refresh-system.md": [
        ...
        "restore proof",
        "Resource Budgets",
        "A project-owned ChatGPT Site is the frontend sandbox",
        "1:1 runnable representation",
        "Localhost and port 7777 cannot satisfy frontend review",
        "variable name only",
    ...
    "references/chatgpt-site-review.md": [
        "1:1 Prototype Gate",
        "Missing evidence blocks Site deployment or acceptance",
    ],
    "references/daily-context-contract.md": [
        "bounded TP-selected Cloud backup",
        "frontend sandbox has no unique ChatGPT Site",
    ],
    "references/agent-sanitizer.md": [
        "Classify custody before backup",
        "Ext remains recovery-only",
...
OPERATIONAL_CUSTODY_REQUIRED = {
    ...
    "solvys-run-point": [
        "Preserve the machine custody boundary",
...
DEFAULT_RESOURCE_CEILINGS = {
    ...
    "concurrentLocalImplementationTasksAtCriticalCapacity": 0,
    ...
    "peakRamPercent": 65,
    "sustainedRamPercent": 55,
    "sustainedRamMinutes": 5,
    "taskOwnedCpuPercent": 60,
    "taskOwnedCpuSustainedMinutes": 3,
    "windowServerGpuProxyPercent": 25,
    "chatgptSitePerTask": 1,
...
def main() -> int:
    ...
    for name, path in OPERATIONAL_SKILLS.items():
        ...
        if name in {"solvys-brief", "solvys-orchestrate"}:
            for field in (
    ...
    try:
        ...
        if (
            execution_lanes.get("frontendSandbox") != "project_owned_chatgpt_site"
            or execution_lanes.get("localhostProofAllowed") is not False
            or execution_lanes.get("extExecutionAllowed") is not False
            or execution_lanes.get("internalLocalAtCriticalCapacity") != "blocked"
        ):
            errors.append("storage policy: Site or local admission lane drifted")
        authority = policy.get("authority", {})
        if (
            authority.get("localControlRoot") != "/Users/tifos/.codex"
            or authority.get("writableSkillsSource")
            != "/Users/tifos/Documents/Solvys/Codebase Cabinet/solvys-skills"
            or authority.get("globalSkillsCompatibilityLink")
            != "/Users/tifos/.codex/tools/solvys-skills"
            or authority.get("extMode") != "restored_writable_recovery_only"
        ):
            errors.append("storage policy: local authority or Ext recovery boundary drifted")
        site_review = policy.get("chatgptSiteReview", {})
        if (
            site_review.get("requiredForFrontendSandbox") is not True
            or site_review.get("uniqueSitePerProjectSandbox") is not True
            or site_review.get("localhostOrPort7777IsAcceptanceProof") is not False
            or site_review.get("automaticHumanReviewWhenVisualOrContentReviewIsNeeded")
            is not True
        ):
            errors.append("storage policy: ChatGPT Site review contract drifted")
        ...
        if backup != {
            ...
            "soundLibrariesDefaultCustody": (
                "ext_recovery_only_pending_explicit_transfer"
            ),
    ...
    try:
        ...
        if registry.get("authorityState") != "local_factory_instance_required":
            errors.append("tranche registry: distributable canon must point to a local Factory instance")
        if registry.get("localRegistryDefault") != "~/.config/solvys-factory/registry/tranche-registry.json":
            errors.append("tranche registry: local registry default drifted")
