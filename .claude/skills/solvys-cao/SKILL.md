...
5. For design work: the complete anti-slop law, shared `solvys-skills/Design.md`, then repo-local `Design.md` or `DESIGN.md`.
...
For installed David Ondrej or Emil Kowalski skills, read `solvys-skill-override` before applying their specialist instructions. David's skills replace legacy Solvys orchestration mechanics; Emil's skills replace the legacy motion-craft guidance. This CAO retains authority for outcome ownership, scope, safety, client partitions, protected zones, proof, and durable learning.
...
and large dependency installs when they would contend for I/O. Do not create a
new heavyweight worktree or cache when the capacity reservation would be
breached. Preserve dirty or active work and ask before any cleanup or move that
falls outside the authorized task.
...
For new frontend changes, use
[wonder-frontend-sandbox.md](references/wonder-frontend-sandbox.md) when the
surface can be represented in Wonder. Wonder is the collaborative provisional
sandbox. Accepted work enters product source only after TP authorizes the
transfer, then port 7777 provides source-integrated verification. TP may edit
Wonder concurrently; disregard human-owned changes outside the agent's recorded
artboard and scope.
...
- For UI, the accepted Wonder decision when applicable, source-transfer
  authorization, port 7777 verification, the complete anti-slop recheck, and
