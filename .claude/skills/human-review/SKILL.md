---
name: human-review
description: Open an HTML file, Markdown file, or localhost page in the browser so the user can edit text directly and leave comments on specific parts, then send all edits and comments back to you. Use automatically for a Site-derived local HTML review artifact when a frontend ChatGPT Site needs visual or content review, and after writing or updating a readable artifact.
---

# human-review

The user reviews your HTML, Markdown, or Site-derived local HTML artifact in a real browser: they fix small things
by typing, select anything to comment on it, and send you the whole batch at once.

For a ChatGPT Site review, the local artifact must represent the accepted Site
source at the same requested scope. It must preserve the real copy, geometry,
states, controls, and responsive target. Do not use human-review to approve a
placeholder, screenshot, fake dashboard, dead control, invented data, or
close-enough substitute. Recheck the actual Site URL after applying feedback.
...
1. Write or update the HTML or Markdown file, or generate the local review artifact from the project ChatGPT Site source.
2. Open it for the user:
   ...
   For frontend work, create a named local HTML review artifact from the Site
   source or export. Check the real Site URL separately in the Codex in-app
   browser. Do not use port 7777 as the normal test or review target:

   ```sh
   human-review /absolute/path/to/site-review.html
   ```
