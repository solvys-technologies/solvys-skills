...
# fallback Cursor agent session to work on it.
#
...
#   model   — Cursor agent model (default: $CURSOR_AGENT_MODEL or gpt-5.4-mini-medium)
#
# Env:
...
if [[ "$1" == "--help" || "$1" == "-h" || $# -eq 0 ]]; then
  ...
  echo "  Cheap/fast:   gpt-5.4-nano-medium, gpt-5.4-mini-medium"
  echo "  Balanced:     gpt-5.4-mini-medium (default unless CURSOR_AGENT_MODEL is set)"
  echo "  Other options: any Cursor-supported agent model available in your environment"
  echo ""
  echo "Environment:"
...
- Read any existing AGENTS.md, WORKSPACE.md, legacy CLAUDE.md, and .cursor/rules/ in the repo before coding
...
echo "→ Launching fallback Cursor agent (model: $MODEL)..."
echo ""
echo "═══════════════════════════════════════════════════════════════════════"
